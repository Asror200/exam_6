...
class BookView(View):
    ...
    def get(self, request):
        ...
        if filter:
            if filter == 'every_writer_is_count_of_books':
                authors = Author.objects.annotate(book_data=Count('books')).order_by('-book_data')
                context = {
                    'authors': authors
                }

            elif filter == 'every_writer_is_most_valuable_book':
            ...
            elif filter == 'every_writer_is_cheapest_book':
                authors = Author.objects.annotate(book_data=Min('books__price')).order_by('-book_data')
                context = {
                    'authors': authors
                }

            elif filter == 'the_average_of_each_writer_is_cheapest_book':
